...
def init_yt_mapping():
  ...
  # 步驟 2：掃描 data/ 下所有歷史每日榜單 CSV，彙整歷史所有不重複歌曲
    ...
    try:
      ...
      if "歌名" in df_hist.columns and "歌手" in df_hist.columns:
        ...
        for _, row in df_hist.iterrows():
          ...
          s_singer = str(row.get("歌手", "")).strip()
          if not s_name or not s_singer:
            continue
          key = (s_name, s_singer)
  ...
  # 步驟 3：建立對照字典，判斷哪些歌曲需要補抓或重新查詢
  existing_mappings = {}
  for _, row in df_mapping.iterrows():
    s_name = str(row.get("歌名", "")).strip()
    s_singer = str(row.get("歌手", "")).strip()
    v_id = str(row.get("Video ID", "-")).strip()
