        ...
        if all_dfs:
            ...
            if song_col and singer_col:
                ...
                for (song, singer), sub_df in df_range.groupby([song_col, singer_col]):
                    ...
                    # 💡 【新增：語言過濾機制】優先從當日前端 CSV 抓語言，若無則從中央對照表反查
                    song_lang = None
                    if "語言" in sub_df.columns:
                        valid_lang_df = sub_df[
                            ~sub_df["語言"]
                            .astype(str)
                            .str.strip()
                            .isin(["-", "nan", "None", "", "未知"])
                        ].dropna(subset=["語言"])
                        if not valid_lang_df.empty:
                            song_lang = valid_lang_df["語言"].iloc[-1]
...
with main_tabs[2]:
    ...
    st.markdown("輸入歌名與歌手後自動帶入舊資料。更新寫入後將自動清空，方便繼續輸入下一首。")

    mapping_file = "data/yt_mapping.csv"

    # 1. 初始化 Session State (綁定歌名與歌手欄位)
    ...
    with col1:
        target_song = st.text_input("🎵 輸入歌名", placeholder="例如：晴天", key="m3_song")
    with col2:
        target_singer = st.text_input("🎤 輸入歌手", placeholder="例如：周杰倫", key="m3_singer")
    with col3:
        st.write("")  # 垂直對齊留白
        st.write("")
        if st.button("🧹 清空重置", key="m3_clear_btn", use_container_width=True):
            clear_m3_inputs()
            st.rerun()
    ...
    with st.form("update_song_form"):
        ...
        with col5:
            new_language = st.selectbox(
                ...
                index=default_lang_idx if has_target else 0,
                disabled=not has_target
            )
